...
function htmlTable(rows: TrackerRow[], columns: ColumnDefinition[]): string {
...
export function clipboardPayload(rows: TrackerRow[], columns: ColumnDefinition[] = DEFAULT_COLUMNS): ClipboardPayload {
  const header = columns.map((column) => column.label).join('\t')
  const plain = [header, ...rows.map((row) => columns.map((column) => columnValue(row, column)).join('\t'))].join('\n')
  return { html: htmlTable(rows, columns), plain }
}
